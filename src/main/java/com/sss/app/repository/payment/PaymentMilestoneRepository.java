...
public interface PaymentMilestoneRepository extends JpaRepository<PaymentMilestone, Long> {
    ...
    // with both billed (amountBase) and collected (amountPaidBase) totals so
    // the same query backs the donut, the KPI totals, and the overdue card.
    @Query("SELECT pm.status, COUNT(pm), SUM(pm.amountBase), SUM(pm.amountPaidBase) "
    ...
    @Query("SELECT COALESCE(SUM(pm.amountPaidBase), 0) FROM PaymentMilestone pm "
